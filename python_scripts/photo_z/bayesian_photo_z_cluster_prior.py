...
from .sed_template_cluster_prior import prior_dict
...
def bin_array_in_xbins(array,x,xbin_edges):
    ...
    # mean_cl, _, _, binned_cl = bin_array_in_xbins(mag_cl, mag_cl, mag_bins)
    '''
...
# ning 250813
def binned_mag_weight(table, filter_map, z_spec_header, reference_mag='i', bins=0.5, inspectBin=True):
    print("========in binned_mag_weight==========")
    mag = table[filter_map[reference_mag]] 
    spec_z = table[z_spec_header]
    cluster_member_mask = (spec_z >= 0.03) & (spec_z <= 0.12)

    mag_min = np.nanmin(mag)
    mag_max = np.nanmax(mag)
    mag_bin_edges = np.arange(mag_min, mag_max + bins, bins)

    # Use helper function to get boolean masks for each mag bin
    _, _, _, binned_array = bin_array_in_xbins(mag, mag, mag_bin_edges)

    if inspectBin:
        for i in range(binned_array.shape[1]):
            bin_mask = binned_array[:, i]
            bin_rows = truncated_table[bin_mask]
            print(f"Bin {i}: mag in [{mag_bin_edges[i]:.2f}, {mag_bin_edges[i+1]:.2f})")
            print(f"Number of objects in this bin: {len(bin_rows)}")
            print("Sample mags:", np.array(bin_rows[filter_map[reference_mag]])[:10])  # first 10 mags
            print("-" * 40)

    n_cl = np.sum(binned_array & cluster_member_mask[:, None], axis=0)
    n_bkg = np.sum(binned_array & (~cluster_member_mask)[:, None], axis=0)

    # ratio per bin
    ratio = n_cl / (n_bkg + n_cl)

    mag_bin_centers = 1/2 * (mag_bin_edges[:-1] + mag_bin_edges[1:])

    def sigmoid_model(x, L, k, x0):
        return L / (1 + np.exp(-k * (x - x0)))

    # Initial guess for L, k, x0
    p0 = [max(ratio), 1, np.median(mag_bin_centers)]

    # Fit the model
    popt, pcov = curve_fit(sigmoid_model, mag_bin_centers, ratio, p0)
    sigmoid_fit = lambda x: sigmoid_model(x, *popt)

    # Display fit plot
    sigmoid_display = sigmoid_fit(mag_bin_centers)

    pl.scatter(mag_bin_centers, ratio, color='blue', label='Data (n_cl/n_bkg)')
    # pl.plot(mag_bin_centers, sigmoid_display, color='red', label='Poly fit')
    pl.xlabel('i-band magnitude')
    pl.ylabel('Ncl/(Nbkg+Ncl)')
    pl.title('cluster prior weight vs. i-band magnitude')
    pl.grid(True)
    pl.savefig(output_directory + "cluster_prior_weight_vs_i-mag.png")
    
    return sigmoid_fit



def compute_statistics(table,template_list,trans,filter_map,error_tag,cluster_prior_weight,cluster_prior=None,cluster_prior_weights=None,z_spec_header=None,filter_template_zps=None,interp_steps=0,reference_mag='i',cosmo=None,redshifts=np.arange(0.01,1.5,0.001),estimator='mode',delta=0.1):
    '''
    ...
    for i in range(total_templates):
        ...
        redshift_prior[:,:,i] = (1-weight)*sed.get_redshift_prior(table[filter_map[reference_mag]],cluster_prior_weight) + weight*sed_upper.get_redshift_prior(table[filter_map[reference_mag]],cluster_prior_weight)
...
def large_sample_quality_check_plots(redshift_statistics,table,filter_map,z_spec_header=None,output_filepath='',blendedness_header='blendedness',refmag='i'):
    ...
    # plotting cat-outlier rate, median_z[:-1], median_mag[:-1]
    ax1.fill_between(median_z,outlier_rate_per_zbin+errorbars_zbin,outlier_rate_per_zbin-errorbars_zbin,alpha=0.5)
    ax2.fill_between(median_mag,outlier_rate_per_mbin+errorbars_mbin,outlier_rate_per_mbin-errorbars_mbin,alpha=0.5) 
    ...
    # median_chi2[:-1]
    ax.fill_between(median_chi2,outlier_rate_per_chi2+errorbars_chi2,outlier_rate_per_chi2-errorbars_chi2,alpha=0.5)
...
def generic_quality_check_plots(redshift_statistics,table,filter_map,z_spec_header=None,output_filepath='',blendedness_header='blendedness',refmag='i',mod_chi2_cut=4,odds_cut=0.95):
    ...
    best_template_index = redshift_statistics['template_at_z_phot']
    print("max min of best_template", np.max(best_template_index), np.min(best_template_index))
    ...
    # first, generic zs vs zp plot + colorbar for reduced chi-2
    fig,ax = pl.subplots()
    # changed todo: c=best_template_index, changed to mod_chi2 for grad school apps 12/26, set_ylabel
    im = ax.scatter(x=z_spec,y=z_phot,s=5,c=mod_chi2,alpha=0.7,vmin=0,vmax=20,cmap='Spectral')
    cb = fig.colorbar(im)
    ...
    # setting labels
    cb.ax.set_ylabel('$ \\chi_{m}^2 $',rotation='horizontal',verticalalignment='center',size=12)
    # cb.ax.set_ylabel('best_template_index',rotation='vertical',verticalalignment='center',size=12)
    ax.set_xlabel('$ z_s $')
    ax.set_ylabel('$ z_p $')
    ax.set_title(f'{num_photoz} Galaxies: NMAD={NMAD:.3f} and $ \\eta $={cat_outliers_percent:.1f}%')
    ...
    bin_edges = np.arange(0.01,1,0.003)
    fig,ax = pl.subplots()
...
def photz_v_specz_template_before_qc(redshift_statistics,table,filter_map,refmag='i',z_spec_header="z_spec",output_filepath=''):
    # computing the remaining statistics
    z_phot = redshift_statistics['z_phot']
    ref_mag = table[filter_map[refmag]]
    # blendedness = table[blendedness_header]
    z_spec = table[z_spec_header]
    best_template_index = np.floor((redshift_statistics["template_at_z_phot"] + 1 ) / 3)
    
    g = table['g_cmodel_mag']
    r = table['r_cmodel_mag']
    g_r = g - r
    # best_template_index = redshift_statistics['template_at_z_phot']
    print("max min of best_template", np.max(best_template_index), np.min(best_template_index))
    ...
    # first, generic zs vs zp plot + colorbar for reduced chi-2
    fig,ax = pl.subplots()
    im = ax.scatter(x=z_spec,y=z_phot,s=5,c=g_r,alpha=0.7,vmin=0,vmax=4,cmap='Spectral')
    cb = fig.colorbar(im)
    # cb.ax.yaxis.set_major_formatter(ticker.FormatStrFormatter('%d'))
    # c=mod_chi2
    ...
    # setting labels
    # cb.ax.set_ylabel('$ \\chi_{m}^2 $',rotation='horizontal',verticalalignment='center',size=12)
    cb.ax.set_ylabel('g-r',rotation='vertical',verticalalignment='center',size=12)
    ax.set_xlabel('$ z_s $')
    ax.set_ylabel('$ z_p $')
    ax.set_title(f'{num_photoz} Galaxies: NMAD={NMAD:.3f} and $ \\eta $={cat_outliers_percent:.1f}%')
    fig.savefig(output_filepath + 'photz_v_specz_g-r.png',bbox_inches='tight',dpi=720)
...
if __name__ == '__main__':
    ...
    cluster_prior_weight = binned_mag_weight(truncated_table,filter_map,z_spec_header)
    # cluster_prior_weight = 1.
    ...
    def compute_statistics_parallel(x):
        # print("compute stats para", z_spec_header)
        out = compute_statistics(x,template_list,trans,filter_map,error_tag,cluster_prior_weight,z_spec_header=z_spec_header,interp_steps=2)
        print("Exiting pool...")
        return out
    ...
    if z_spec_header is not None:
        # photz_v_specz_template_before_qc(redshift_statistics,table,filter_map,output_filepath=output_directory)
        ...
        large_sample_quality_check_plots(redshift_statistics,table,filter_map,z_spec_header=z_spec_header,output_filepath=output_directory)
