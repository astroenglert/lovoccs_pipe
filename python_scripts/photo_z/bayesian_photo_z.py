...
def compute_F(sed_template,magnitude_table,filter_map,error_tag,sed_template_upper=None,weight=None,filter_template_zps=None):
    ...
    for i in range(len(filter_map)):
        ...
        # TODO: set as sys args
        apply_ext_err = False
        if apply_ext_err: 
            ext_unc = 0.1
            print(f"APPLIED EXT_ERROR of {ext_unc} on filter", transmission_filter)
            # from lovoccs spreadsheet A2029, assume 20% uncertainty
            extinction = {'u':0.169, 'g':0.132, 'r':0.091, 'i':0.068, 'z':0.05}
            ext_error = ext_unc * extinction[transmission_filter]
        else:
            ext_error=0
        ...
        flux = 10**(-0.4 * mag)
        # mag_error is actually zero_point error
        flux_error = ( np.log(10)/2.5 ) * np.sqrt(mag_error**2 + template_error**2 + ext_error**2) * flux
