...
weight = 0.5
# redshift_prior = redshift_prior1 * (1-weight) + cluster * (weight)
print("--prior weight: ", weight)
...
class TemplateBase(ABC):
    ...
    def _hubble_redshift_priors(self,m0,galaxy_type,z=None):
        ...
        if z is None:
            z = self.redshifts
        print("z in hubble_redshift_priors:", z)
    ...
    def _cluster_prior(self, sigma, z):
        cluster_z = 0.093
        tmp = np.exp(-(z-cluster_z)**2/(2.*sigma**2))/(sigma*(2.*np.pi)**0.5)
        return tmp

    def _plot_prior(self, z, prior):
        plt.plot(z, prior)
        plt.xlabel("spec-z")
        plt.ylabel("Prior probability p(z|T,m0)")
        plt.title("Example prior distribution for T=1")
        print("type1 gal, prior figure saved")
        plt.savefig("A2029 type1 prior")

    def _ngvcs_redshift_priors(self,m0,galaxy_type,cluster_prior_weight,z=None):
        ...
        tmp = self._cluster_prior(0.03, z)
        print(cluster_prior_weight)
        if galaxy_type == 1:
            for i in range(4):
                m0_subset = m0[conditions[i]]
                if len(m0_subset) == 0: continue
                amp = self._hubble_redshift_prior_amplitude(early_alpha[i], early_z0[i], early_kmt[i], m0_subset, ref_mag[i])
                output_arr[conditions[i]] = amp[:,None]*early_prior[i](m0_subset) * (1. - cluster_prior_weight) + tmp * (cluster_prior_weight)
                # self._plot_prior(z, output_arr[conditions[i]][0]) 
            
        elif galaxy_type == 2:
            for i in range(4):
                m0_subset = m0[conditions[i]]
                if len(m0_subset) == 0: continue
                amp = self._hubble_redshift_prior_amplitude(late_alpha[i], late_z0[i], late_kmt[i], m0_subset, ref_mag[i])
                output_arr[conditions[i]] = amp[:,None]*late_prior[i](m0_subset) * (1. - cluster_prior_weight) + tmp * (cluster_prior_weight)
            ...
            for i in range(4):
                m0_subset = m0[conditions[i]]
                if len(m0_subset) == 0: continue
                amp = self._hubble_redshift_prior_amplitude(irr_alpha[i], irr_z0[i], irr_kmt[i], m0_subset, ref_mag[i])
                output_arr[conditions[i]] = amp[:,None]*irr_prior[i](m0_subset) * (1. - cluster_prior_weight) + tmp * (cluster_prior_weight)
                # np.zeros(len(z))
...
class ElB2004a(TemplateBase):
    ...
    def get_redshift_prior(self,m0,cluster_prior_weight=weight,redshift=None):
        return self._ngvcs_redshift_priors(m0,1,cluster_prior_weight,z=redshift)
...
class SbcB2004a(TemplateBase):
    ...
    def get_redshift_prior(self,m0,cluster_prior_weight=weight,redshift=None):
        return self._ngvcs_redshift_priors(m0,2,cluster_prior_weight,z=redshift)
...
class ImB2004a(TemplateBase):
    ...
    def get_redshift_prior(self,m0,cluster_prior_weight=weight,redshift=None):
        return self._ngvcs_redshift_priors(m0,3,cluster_prior_weight,z=redshift)
...
# ning 250818, check by single template
default_sed_list = [
                    # ElB2004a
                    # SbcB2004a,
                    # ScdB2004a,
