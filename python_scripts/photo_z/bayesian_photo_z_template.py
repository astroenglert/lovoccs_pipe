...
from .sed_template_template import prior_dict
...
def compute_F(sed_template,magnitude_table,filter_map,error_tag,sed_template_upper=None,weight=None,filter_template_zps=None):
    ...
    for i in range(len(filter_map)):
        ...
        # print(transmission_filter)
        # if transmission_filter in ['u','g']:
        #     template_error = 0.002*2
        # else:
        #     template_error = 0.002    
        template_error = 0.002
        ...
        flux = 10**(-0.4 * mag)
        flux_error = ( np.log(10)/2.5 ) * np.sqrt(mag_error**2 + template_error**2) * flux
...
def compute_statistics(table,template_list,trans,filter_map,error_tag,cluster_prior=None,cluster_prior_weights=None,z_spec_header=None,filter_template_zps=None,interp_steps=0,reference_mag='i',cosmo=None,redshifts=np.arange(0.01,1.5,0.001),estimator='mode',delta=0.1):
    '''
    ...
    # print("<< new cosmos 134 templates", sed_len)
...
if __name__ == '__main__':
    ...
    output_directory = output_directory + prior_choice + '-'
