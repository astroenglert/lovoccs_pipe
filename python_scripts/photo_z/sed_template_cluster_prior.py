    ...
    def _ngvcs_redshift_priors(self,m0,galaxy_type,cluster_prior_weight,z=None):
        ...
        if galaxy_type == 1:
            for i in range(4):
                ...
                output_arr[conditions[i]] = amp[:,None]*early_prior[i](m0_subset) * (1. - cluster_prior_weight(m0_subset)[:, None]) + tmp * (cluster_prior_weight(m0_subset)[:, None])
            ...
            for i in range(4):
                ...
                output_arr[conditions[i]] = amp[:,None]*late_prior[i](m0_subset) * (1. - 0) + tmp * (0)
            ...
            for i in range(4):
                ...
                output_arr[conditions[i]] = amp[:,None]*irr_prior[i](m0_subset) * (1. - 0) + tmp * (0)
                # np.zeros(len(z))
