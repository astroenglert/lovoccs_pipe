...
# there are some shortcomings of my regular mass_map script
# 1. WCS is probably offset/incorrect
# 2. The call back to the coadd isn't great
# 3. Breaks beyond ~1deg when the TAN projection starts to break down (difference on order of ~1-10%)

# How can I write a more robust on-sky version of this?
# 1. I can define a NEW WCS to use centered at the X-ray luminosity (arbitrary choice technically)
# 2. Convert catalog coordinates to the new-system; define grid in that projection to sample mass_map
# 3. Compute MapE/B/V and define an upscaled WCS for the mass_map
# That will work... but will still break-down ~1deg beyond the cval
# BUT, the simplest solution I can think, is to compute MapE/B/V using a locally flat sky centered at the sampled-coordinate, and then space those sampled-coordinates in whatever system is convenient (e.g. a square centered on the x-ray peak). Defining a WCS object, converting into that system, then doing the distances isn't efficient.
# Better yet, the local flat-sky with the correct factor will work. E.g., at a given point I want to sample (ra_1, dec_1), for the distance use
# delta_theta = sqrt( delta_dec^2 + delta_ra^2 * cos(dec_1)^2 )
# this will be valid relative to each point at least out to Rs ~1deg, likely beyond since, even though the approximation breaks down, the outer-most terms are severely downweighted by the filter so their contribution is negligible
# and a small number of extra operations! All I need to do is change the sampling and how the radius is computed
