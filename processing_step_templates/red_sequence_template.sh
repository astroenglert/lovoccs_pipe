...
echo "mapping the rs..."
python python_scripts/red_sequence/red_sequence.py "shear_calibration_output/${CLN}_dered_dezp_zphot_scal_gals.csv" "${COADD}" "100" "500" "red_sequence_output/" "decam" #"3,3" assumed, needs to be specififed if using a different set of patches

# and finally render the density
echo "rendering contours..."
python python_scripts/render_data/overlay_contours_map_rs_inv.py "${COADD}" "red_sequence_output/rs_density_kde.fits" "mass_map_output/Map_E_peak_catalog_schirmer.csv" "mass_map_output/" "red_sequence_output/${CLN}_r_inv_Map_rs.png"
